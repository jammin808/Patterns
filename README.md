...
- **Lower thirds, an engine inside Patterns** — a design is a box of elements anchored on the
  canvas: text (a name, a role, a company, the date, the time, or your own words), bars,
  ...
  way in and out as editable keyframes (offset, opacity, scale, rotation, a wipe) with easing,
  staggered one after another. Designed once at 1080 lines, it draws on every sink — displays,
  a joined canvas, NDI, the stream, the thumbnails — from the same clock, reacts to effect
  stings like the full-screen studio, and ten presets (Clean, Broadcast, Glass, Neon,
  Corporate, Tag, Headshot, Sparks, Fractal, Stamp) are one click from a starting point.
