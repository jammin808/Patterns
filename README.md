...
```bash
dotnet test                      # 946 tests: Companion 2.0 and OSC (the look-by-place verb parsed and mapped, the feedback for every list and its blanks, the deck and page rows and their empties; on a live desk two looks going out by place with the one on air marked, LOOK #2 / #1 / 1 / a name all landing, #5 refused with the count, the look in the preview, the pattern on air, the same facts as OSC feedback), edge blend beyond two projectors (rows of three and four, a stack, a 3×2 grid, the joining through one blending projector, zones that do not fit, the audit's every verdict, the grid pattern's markers and grey; on the real pipeline three projectors' joins and a 2×2 grid's shared corner summing to white), PowerPoint decks through LibreOffice (the extensions and kinds, the cache name that follows the file, the command line, the search order; on a live desk a stand-in LibreOffice held at a gate — the pending deck on the desk, the wire and STATE, the PDF taking its place with the pages on the PREVIEW pane, the cache serving the next mount, RELOAD and an edited file converting afresh — the honest card and the path box without LibreOffice, the real LibreOffice converting an Impress deck when the machine has one), PDF decks (the page words and rasters; the deck wanted and its page drawn at its own shape with bars; the wire and OSC; the cue action through spec, checks, summary and sheet; on a live desk the real renderer opening a three-page deck, the pages on the PREVIEW pane, the desk, the keyboard, the wire and a cue turning them, the hold at the last page and the GO of the standby cue at the end, the start page, a missing file's card and the Media page's block), web pages that answer (key chords as a browser reads them; the services' full-frame addresses and their own actions; the WEB verbs on the wire and over OSC; the cue action through the spec, the checks, the summary and the sheet; on a live desk the verbs reaching the page on air and a named page, YouTube's player, a cue's action, the state row, KEYS → PAGE taking the desk's F5 and Space and giving them back, FULL FRAME and the pages' blocks), the area of interest (the crop reaching 90 % of a side and keeping a twentieth, composed by a second pick; a still cropped, mirrored and turned on the output; a live feed cropped through its source and a page keeping its clicks through the crop; the box on the PREVIEW pane, the presets, the clear), the multiview's tally (the badges for program, the next take, held, locked, own, a repeater, off, a blackout and a freeze; the captions naming a screen, a canvas, an NDI send and the program's and the preview's screens; the chips painted on the tiles and gone without the tally; the wall's arming reaching both snapshots and the remotes), lower thirds triaged (a design drawn on the output, the NDI send, the stream and the desk's panes and gone when hidden, the show's default and the in-place put, the preview / take / update verbs on the wire, in the cues and over OSC; on a live desk an AIR again and an UPDATE carrying an edit to the air copy, a picture TAKE keeping the lower third where it is, the sign-off flow refused without EDIT SAFE, the default taking the people, the pages' controls), freeze, the timed fade, the previous look and the show's earlier versions (a frozen output holding while a monitor moves and a blackout takes it, the verbs and the seconds, the store's versions, the desk and the wire), the walkthroughs (the catalogue whole, the progress machine, every step a shell page and every check answered on a live desk, GO, the show ticking steps by itself, the hand ticks, the page), bezels and gaps (the maths, the rig, content across the strips, the output cut, a monitor shading, the LED and video wall tiles on the real panels, the page), review on the multiview (a Preview tile beside the program, following the sandbox, the review filling the wall, the slate without a sandbox, the verbs, the flag runtime only), the phone remote (its seven tabs, the tab remembered, no outside script, every command line and the client header, the two-press STOP ALL, the state the tabs read), the watchdog and the beacon (the beacon both ways and a stray packet that is not one, the watch waiting, seen, silent and down, the advisor seeing frozen outputs and a stopped stream, the stand-down note read once onto the health line, the super-check rows, the heartbeat going out and a main machine heard with this machine's own beacon ignored, the supervisor's last word), OSC in and out (the wire format for every type, padding, bundles and bad packets; every address onto its line in every spelling and every line a command the protocol knows; the state out as messages and a bundle round trip; real datagrams against the live app — a command changing the show and the feedback bundle saying so, pong, an error for a refused command and for a stranger address, the status reply, a bundle in order, the switch off), the lower-thirds library (an entry filling a design's fields and its picture with the picture home chosen by name, the library found by id, name and number, a speaker list in every spelling with first and last names joined and a nameless row noted, the template, a merge that updates and appends, an export round trip, the cue action with a person summarised, checked and refused, the sheet's value, the remote verbs, the page keeping people with USE and SHOW, PERSON and LT … WITH … from the remote, a stranger refused, the panel's chips, a list imported and appended, a cue naming a person fired on air and through the sandbox), the caller's home (clock and length spellings, a sheet becoming cues with looks, times, marks and actions, the template and an export round trip, the day estimated from the plan and the clock, the caller's edits, a sheet imported and appended from the Cues page, the Run surface reading the clock, a follow firing through the gate and stopped by the caller, CSV and the first sheet of an .xlsx), web pages inside the engine (the show wanting its pages once, the page drawn as a pattern and as a layer with its box for the desk and the pointer and its ripple when asked, the maths through a crop both ways, the engine mounting pages with zoom live and a new viewport reopening, a click on the PREVIEW pane reaching the page, a web layer clicked into and dragged, the page controls typing), layers and dragging (a layer in its box with a border, corners and an opacity, an empty one seen on the desk and not on an output, the pane map finding what is under the pointer through two letterboxes, a move that never fades and a new picture that does, layers wanting their inputs and showing another screen, the PREVIEW pane dragging a layer and the clock with the one on top winning, a sandboxed drag that moves the preview and not the air, the designer picking and dragging an element), screens with a job (a lock holding through every look, TAKE ALL and a clip's takeover, a canvas locked through its members, repeaters and chains that never hang, the Screens page and the wall driving roles, locks and mirrors, LOCK from the remote and a cue, SEND to one tile), the desk's hints (folded away by default, back with the tick, remembered; ? TIPS reading a page under its headings), PREP holding the stream, MODE and LAYOUT apart, the type stepped up; pixel-exact rendering, the ticker's seamless loop, the stop fade, the gain buses, the live duck, edge blend, the PiP crop, frame pacing, capture formats, a screen's rename across a mode change, the library's sections and search and its schema-7 upgrade, particle coverage under wind and every scene pack, every fractal family on both render paths and the sound analysis behind it, effect stings through particles and fractals (every shape settles, the scored phases, the sliders that write back), lower thirds (every ease, keys blended in order, the clock through in, hold and out by the hold or by a hide, the motions' keys and the stagger, ten presets inside their boxes and through the show's JSON, the picture in its box on every sink and gone cleanly, every element kind drawn with its caches swept, a media element's clip wanted while it is on; the remote verbs, the cue action checked and summarised, a look that carries one and recalls it afresh, a design as a file; the designer's edits, the file round trip, SHOW and HIDE from the desk, the remote and through the sandbox, the tally and the panel chips, the preview at three instants), direct output (the decision from every fact, a card that must be hardware, the honest status lines, the fuse armed and disarmed across a start and held after a failed one, a window prepared live and put back, the tick and the summary on the pages), the master clock (wall time that never steps, a drift measured to the ppm, a lock that holds a fast device to a few milliseconds where free-running loses ten, the converter transparent at one and clean at a pitch, the delay line, the sync marks and the clicks landing on the frame, the video delay reaching every clip, the stream's option), the tally (the look on air, edited, in the preview, taken; a playing VOG and a sting's bar), the desk's dividers (moved, clamped, held back on a small window, WIDE, remembered in the show), the super-check (a strong machine all clear, a laptop on battery with an idle card, the red rows, unknown facts grey, the level, the report on the page and in the file), the feeds' own screens (one per send, the stream's while set to it, never joined to a canvas, a mirrored target's shape kept in the frame, an own screen's look, the frame feed that never tears, the rendered stream plan), arrangement math, target geometry, playlists, input pool, DSP, remote protocol, watchdog policy, VOGs and stingers (the split, the fade, every after-policy, the hold), break music (Spotify, offline through a fake transport: the verbs, browsing and search, music on a look), switcher, sandbox/air routing, prep mode + screen adoption, playlist parts, multiview pixels, stream MRLs, GPU selection, health advisor, metrics, headless UI
