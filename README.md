...
- **The handed-in plan's Phase 0 (round 85)** — nine reported defects read against the head, seven fixed:
  the replay's fault counts read as the minute's own; the replay's record read on a worker, bounded and
  stoppable, the picture relit when it lands; a recovery record that parses to nothing reads its backup,
  and a torn record is never rotated over the whole one; a second desk that comes to own a newer build's
  show file keeps saving off; a bind that is not an address closes the control ports, OSC and the audience
  socket with the reason, never every interface; the remote, admin and multiview pages carry the token on
  every read, and every `/api/cmd` line and `/api/cues` want it on a paired desk. Two designed as ledger
  rows: the stream's rate held through a display change; one set of output facts. The second look at the
  connectomics transfer: `docs/CONNECTOMICS.md` §6; the design: `docs/PLAN.md` §103.
- **The Eye replayed, and synthetic shows as a fence (round 84)** — REPLAY on the Eye page plays the
  desk's own record back over the picture of now: the journal's rows light the screens, the canvas
  members and the devices they named in the thirty seconds before an instant (green Done, amber Refused,
  red Failed), the desk wears the machine's sample then, and what the record does not mention is grey and
  says so; a scrub bar and two 30 s steps move the instant, NOW comes back, and the rail stays live.
  `EYE REPLAY [ON|OFF|<time>]` and `EYE AT <time>` on the wire; `replay` and `replayAt` in STATE's eye row.
  A fixed-seed generator of rigs — canvases, repeaters, locks, own pictures, ticks, un-armed tiles — runs
  five hundred and twenty shows nobody drew through the take resolver, the show file's round trip, the
  frozen clone and the shown-picture rule, and eight through a live desk's take. The method and the
  transfers: `docs/CONNECTOMICS.md`; the design: `docs/PLAN.md` §102.
- **The ledger's rows that are code (round 83)** — a fault behind the wire is answered `ERR … fault #n, logged`
  on the line and the connection stays (the first fault with its stack, then one a minute, and the count on the
  Super Check); a browser page from another origin cannot post to the desk, paired or loopback or not, while a
  curl or a device is not touched; with a token set the state and the pictures want the token too, by the header
  or the pages' own cookie; a sidecar is flushed to the disk before its move and the recovery record keeps the
  one before it, read when the record is torn and said on the Super Check; the word list matches whole words
  ("Scunthorpe" passes) with an entry's own wildcards saying otherwise; the ticker feed is capped at 2 MB, its
  failure is one line, and its items meet the word list; the NDI receiver refuses a frame whose stride is below
  a row or whose format it never asked for. The design: `docs/PLAN.md` §101; the ledger: `docs/OPEN.md` — six
  rows closed, L42 and L43 added.
- **A handed-in review answered (round 82)** — a review's patch set, written against round 80 and merged as
  this round: a span of time on the wire is finite and at most a week (a slip past a clock threw, an enormous
  hour wrapped to a small one, and TIMER ADD +Infinity was taken); every number the show writes is finite, so no
  word can make the show unsaveable; the four OSC addresses the reference documented and the map refused now
  route, with a fence holding the table to the map; a fault in one OSC message is that message's and the port
  carries on; a phone's nickname and group meet the room's word list before the wall draws them; and the parsers
  that read from outside the desk run under a fixed-seed fuzz in the Core suite. The design: `docs/PLAN.md` §100;
  what is still open: `docs/OPEN.md` L31–L41.
- **The switcher wall's workflow (round 81)** — a group is what a screen is for: TICKED GROUPS takes to
  every screen of the ticked tiles' groups (Main, Confidence, Info) and MAIN / CONFIDENCE / INFO SCREENS to
  a group by name, while a joined canvas is a canvas (`CANVAS A`), never a group; every scope but ALL ARMED
  lands on its screens alone, as their own pictures — the programme and every other screen untouched,
  nothing pinned; a tile on its own picture (OWN) keeps its PVW to itself until SEND copies the programme's
  preview there or the tile is selected and edited, and its CUT / TAKE put up what its PVW shows; the
  picker, the tile's title and the highlight are one selection and FOCUSED means it, on the desk, in
  STATE's take row (`focused`, `groups`) and on the deck (Companion module 3.16.0); LOCK means lock on every
  path.
- **The ledger's rows that need no rig (round 80)** — the master rate follows the displays: a 50 Hz TV
  under a 60 fps show runs the show at 50 by default, every reader reads the rate in force (the outputs,
  NDI, the stream, the chip, the Super Check's *Master rate* row, STATE, the Eye, the deck) and the Screens
  page's FOLLOW THE DISPLAYS switch holds the setting instead; the take plan says before the press which
  armed tile keeps its own picture; Companion module 3.15.0 reads the desk's descriptor version and warns
  on a newer one; one atomic write for every sidecar; an on-demand poll reads the machine now.
- **The retrospective answered (round 79)** — a whole-project retrospective handed in and checked claim
  by claim; every action result now says what the room could see and what changed (`Visibility`,
  `Effect` — in the journal, STATE's `take.last` and the Eye), and a wall TAKE that would change nothing is
  refused; the last fail-open words on the wire and the OSC map refuse; OSC honours the bind address and is
  amber until it does; the action recorder writes every wire verb the Companion module can send, with the
  module's own lines as the oracle; the wall's miniatures and the RUN monitor present at the desk's own
  monitor rate (the Screens page's Desk monitors picker) and LIMITED BY RENDER CLOCK names its cause; every
  build says which build it is; a show file from a newer build runs as read; the support bundle carries
  the playhead and the quality profile; `CLAUDE.md`, `docs/OPEN.md` and `docs/field/` are how a round
  opens and closes from here on. The design: `docs/PLAN.md` §97.
...
- **The small round (round 75)** — a generalisation roadmap read against the code and answered
  ...
  a LOCK during a clip pins the show beneath the clip, never the transition; a `protocol` version on the NAV
  and MENU replies. The design: `docs/PLAN.md` §93.
- **The deck as a programming surface** — a Companion (or any controller on the wire) reads the
  desk's own table (`NAV`: the rails and pages as JSON), turns the desk's pages and selects things
  on them (`NAV Looks Walk-in`, `NAV BACK`, `NAV HOME`, `NAV SETTINGS`), asks for a page's own
  menu (`MENU PAGE Looks`: its looks, cues, designs, screens with their lines and ticks, and the
  page's build verbs), edits the show file from a key (`LOOK SAVE`, `LOOK UPDATE`, `LOOK DELETE`,
  `CUE ADD`, `CUE DELETE`, `PRESET SAVE`, `LT NEW … FROM Neon`), says where it is (`NAV DECK`, on
  the Remote page and in the Eye) and records what the operator does on the desk as the lines that
  reproduce it (`RECORD ON`, then `ACTION LOOK Walk-in`). The Companion module 3.14.0 lays all of
  it on 24 slot keys — rails → pages → a page's menu → a thing's menu → drawers, with HOME, BACK,
  PREV / NEXT, RUN / MENU and FOLLOW both ways — feeds Companion's action recorder and a key's
  LEARN from the desk, and steps the levels from Stream Deck + knobs. The dynamic-key answer and
  the design: `docs/COMPANION-NAVIGATOR.md`; `docs/PLAN.md` §92.
...
- **The switcher's rules, the tile, the next take, the group** — CUT / TAKE do what the picker says
  ...
  inside every scope, FOCUSED is the highlighted tile alone, TICKED the ticked alone, TICKED GROUPS every
  screen of the ticked tiles' groups (round 81: a group is what a screen is for), a repeater never — and
  every scope but ALL ARMED lands on its screens alone, as their own pictures (round 81); a take that would move nothing is refused with the reason, and the line under the
...
- **Which group a screen is in** — main screen, confidence monitor, info desk, repeater, an NDI
  ...
  Screens on that screen, where Role and Mirror of are set. Since round 81 TICKED GROUPS means this
  group — every screen of the ticked tiles' kinds — and a joined canvas is a canvas (CANVAS A), never a
  group; the tooltips and Help say so.
...
- **Fade to black on one screen or a group, with the sound** — FADE TO BLACK and FADE UP on the
  ...
  `FADE CANVAS A`, `FADE GROUP CONFIDENCE`, `FADE FOCUSED`, `FADE TICKED`, `FADEUP … SCREEN 2`), OSC too
...
- **Frame rate, display modes, capture formats** — a **master frame rate** for the show (every
  ...
  can follow it; and, by default, the rate **follows the displays** — a 50 Hz display under a
  60 fps show runs the show at 50, the Screens page, the Super Check, STATE and the Eye say which
  display leads and what was set, and the switch holds the setting instead — round 80); a
  **display-mode picker** per screen that lists what the display's EDID offers
...
Working the tree: [`CLAUDE.md`](CLAUDE.md) is the brief — the standing brief, the doctrine, the
constraints, where every artefact lives, how a round opens and closes; [`docs/OPEN.md`](docs/OPEN.md)
is the cumulative ledger of what was found and not yet fixed, with what closes each row;
[`docs/field/`](docs/field/README.md) is where the show laptop's four files go with every report, and
[`docs/QUALIFICATION.md`](docs/QUALIFICATION.md) §22 is the scripted walk that fills a row on the rig.
...
- **Which build is this.** Every build says: the Admin page, the log's banner, STATE's `version`, the
  beacon and the Eye read `0.<round>.<run>+round-NN.<sha>` for a CI build, `0.<round>.0+round-NN.<sha>`
  for a script's, `1.0.0+<sha>` for a plain `dotnet build` — never a bare 1.0.0 — and the manifest
  carries the same round and version.
- **Releases.** Every tag from `round-61` on gets a Release on the repository's Releases page:
  ...
  retention period. From `round-64` a manifest rides beside the exe and on the Release
  (`Patterns-<tag>-manifest.json`): the commit, the runtime, Avalonia, SkiaSharp, LibVLCSharp
  and the libVLC payload's exact version and hash, taken from the package the build restored —
  and `Patterns.exe --verify-runtime` is the bundle's own check (libVLC loads, the audio devices
  answer, Skia draws, the modules read), run by CI on what it built and by you on a show machine
  before the day.
