...
- **The badge goes on every picture the desk draws** — the rule was an allow-list of pattern
  ...
  simply the most recent). It now names the two exceptions — never on a monitor wall, and on media
  only when asked — so the next kind carries it the day it is added.
