...
public class OverlayRemoteTests
{
    ...
    {
        Assert.Equal(ShowActionKind.ClockToggle, ControlProtocol.Parse("CLOCK").Action.Kind);
        Assert.Equal(ShowActionKind.ClockOn, ControlProtocol.Parse("clock on").Action.Kind);
        Assert.Equal(ShowActionKind.ClockOff, ControlProtocol.Parse("CLOCK HIDE").Action.Kind);
        Assert.Equal(new ShowAction(ShowActionKind.ClockFormat, "", "24"), ControlProtocol.Parse("CLOCK 24H").Action);
        Assert.Equal(new ShowAction(ShowActionKind.ClockFormat, "", "12"), ControlProtocol.Parse("CLOCK 12").Action);
        Assert.Equal(new ShowAction(ShowActionKind.ClockSeconds, "", "off"), ControlProtocol.Parse("CLOCK SECONDS OFF").Action);
        Assert.Equal(new ShowAction(ShowActionKind.ClockSeconds, "", "toggle"), ControlProtocol.Parse("CLOCK SECONDS").Action);
        Assert.Equal(new ShowAction(ShowActionKind.ClockDate, "", "on"), ControlProtocol.Parse("CLOCK DATE 1").Action);
        Assert.Equal(RemoteCommandKind.Unknown, ControlProtocol.Parse("CLOCK sideways").Kind);

        Assert.Equal(ShowActionKind.MessageToggle, ControlProtocol.Parse("MESSAGE").Action.Kind);
        Assert.Equal(new ShowAction(ShowActionKind.MessageOn), ControlProtocol.Parse("MESSAGE ON").Action);
        Assert.Equal(new ShowAction(ShowActionKind.MessageOn, "", "Doors open at 7"), ControlProtocol.Parse("MESSAGE ON Doors open at 7").Action);
        Assert.Equal(new ShowAction(ShowActionKind.MessageOn, "", "Doors open at 7"), ControlProtocol.Parse("MSG Doors open at 7").Action);
        Assert.Equal(new ShowAction(ShowActionKind.MessageOn, "", "Welcome back"), ControlProtocol.Parse("MESSAGE TEXT Welcome back").Action);
        Assert.Equal(ShowActionKind.MessageOff, ControlProtocol.Parse("MESSAGE OFF").Action.Kind);
        Assert.Equal(new ShowAction(ShowActionKind.MessageScroll, "", "on"), ControlProtocol.Parse("MESSAGE SCROLL ON").Action);
        Assert.Equal(new ShowAction(ShowActionKind.MessageScroll, "", "toggle"), ControlProtocol.Parse("TICKER").Action);
        Assert.Equal(new ShowAction(ShowActionKind.MessageScroll, "", "off"), ControlProtocol.Parse("TICKER OFF").Action);

        Assert.Equal(new ShowAction(ShowActionKind.CountdownStart, "", "5"), ControlProtocol.Parse("COUNTDOWN 5").Action);
        Assert.Equal(new ShowAction(ShowActionKind.CountdownStart, "", "2.5"), ControlProtocol.Parse("COUNTDOWN START 2:30").Action);
        Assert.Equal(new ShowAction(ShowActionKind.CountdownStart, "", "1.5"), ControlProtocol.Parse("TIMER START 90s").Action);
        Assert.Equal(new ShowAction(ShowActionKind.CountdownStart), ControlProtocol.Parse("COUNTDOWN START").Action);
        Assert.Equal(new ShowAction(ShowActionKind.CountdownTo, "", "19:30"), ControlProtocol.Parse("COUNTDOWN TO 19:30").Action);
        Assert.Equal(new ShowAction(ShowActionKind.CountdownTo, "", "14:00"), ControlProtocol.Parse("COUNTDOWN AT 14:00").Action);
        ...
        Assert.Equal(ShowActionKind.LogoToggle, ControlProtocol.Parse("LOGO").Action.Kind);
        Assert.Equal(ShowActionKind.LogoOn, ControlProtocol.Parse("LOGO ON").Action.Kind);
        Assert.Equal(ShowActionKind.LogoOff, ControlProtocol.Parse("LOGO OFF").Action.Kind);
        Assert.Equal(ShowActionKind.PipToggle, ControlProtocol.Parse("PIP TOGGLE").Action.Kind);
        Assert.Equal(ShowActionKind.PipOn, ControlProtocol.Parse("PIP SHOW").Action.Kind);
        Assert.Equal(ShowActionKind.PipOff, ControlProtocol.Parse("PIP OFF").Action.Kind);
        Assert.Equal(ShowActionKind.OverlaysOff, ControlProtocol.Parse("OVERLAYS OFF").Action.Kind);
        Assert.Equal(RemoteCommandKind.Unknown, ControlProtocol.Parse("OVERLAYS ON").Kind);
        Assert.Equal(new ShowAction(ShowActionKind.PatternKind, "", "LED wall"), ControlProtocol.Parse("PATTERN LED wall").Action);
