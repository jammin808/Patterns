...
public class GlanceTests
{
    ...
    public void ThePlanVerbsAndTheCountdownsFollowParseOnTheWire()
    {
        ...
        Assert.EndsWith(",switchWorstMs,slowSwitches,goWorstMs,lagWorstMs,renderFaults,privateMB,managedMB,liveAgeWorstMs,retiringMB,poolStarved", MetricsCsv.Header);
        Assert.EndsWith(",42.4,3,,,0,,,,,0", MetricsCsv.Line(new MetricSample { Utc = DateTime.UnixEpoch, SwitchWorstMs = 42.4, SlowSwitches = 3 }));
        Assert.EndsWith(",34.5,21.4,0,,,,,0", MetricsCsv.Line(new MetricSample { Utc = DateTime.UnixEpoch, GoWorstMs = 34.5, LagWorstMs = 21.4 }));
        Assert.EndsWith(",,12.5,3", MetricsCsv.Line(new MetricSample { Utc = DateTime.UnixEpoch, RetiringMB = 12.5, PoolStarved = 3 }));
        ...
        Assert.EndsWith(",8.1,2,,0,,,0,,,,,0", MetricsCsv.Line(new MetricSample { P95FrameMs = 8.1, MissedSlots = 2 }));
    }
}
