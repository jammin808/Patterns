...
public class ShellTests
{
    ...
    public void ThePageTableMatchesTheWindowsTabsAndEveryGroupHasPages()
    {
        ...
        try
        {
            ...
            Assert.Equal(new[] { "Screens", "Multiview", "Audio", "NDI", "Stream", "Remote", "Interactive", "Arcade", "Nodes" }, Shell.Pages.Where(p => p.Group == ShellGroup.Setup).Select(p => p.Header));
    ...
    public void GroupsRememberTheirPageAndTheStripFollows()
    {
        ...
        try
        {
            ...
            Assert.Equal(new[] { "Pattern", "Media", "Overlays", "Lower thirds", "Countdown", "Particles", "Fractals", "Reactive", "Branding", "Layers", "Library", "Assistant" }, vm.PageStrip.Select(p => p.Header));
