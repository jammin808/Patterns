...
namespace Patterns.Rendering.Particles;
...
public sealed class ParticleSimCache : IDisposable
{
    ...
    public ParticleSim Get(ParticleOptions o, ShowSnapshot snap, SKSizeI canvas)
    {
        ...
        foreach (var e in _entries)
        {
            ...
            e.Used = _tick;
            snap.Attachments.Get<ParticleLeaders>().Offer(key, e.Sim);
            return e.Sim;
        }
        ...
        entry.Used = _tick;
        var leader = snap.Attachments.Get<ParticleLeaders>().Offer(key, entry.Sim);
