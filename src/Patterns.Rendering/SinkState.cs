...
public sealed class SinkState : IDisposable
{
    ...
    public CheckerShaderCache Checker { get; } = new();

    /// <summary>Parsed colour-cycle list cache.</summary>
    public CycleColorCache CycleColors { get; } = new();
    ...
    public Dictionary<Patterns.Core.Model.ReactiveScene, SKRuntimeEffect> ReactiveEffects { get; } = new();

    /// <summary>Scenes whose shader would not compile here: drawn on the CPU instead, asked for once.</summary>
    public HashSet<Patterns.Core.Model.ReactiveScene> ReactiveUnavailable { get; } = new();
    ...
    public Dictionary<string, Patterns.Rendering.LowerThirds.LowerThirdElementCache> LowerThirds { get; } = new();
