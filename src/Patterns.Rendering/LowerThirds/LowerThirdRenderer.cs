...
namespace Patterns.Rendering.LowerThirds;
...
public static class LowerThirdRenderer
{
    ...
    private static void DrawFractal(SKCanvas c, in PatternFrame f, LowerThirdElement e, LowerThirdElementCache cache, SKRect rect, double time, in EffectSurge surge)
    {
        ...
        {
            cache.FractalColors = FractalPattern.PaletteOf(o.ColorsCsv);   // the same five the pattern's shader reads
        ...
        if (FractalPattern.UsesShader(f.Ctx.Sink))
        {
            var audio = o.AudioSource == AudioSourceKind.None ? AudioLevelFrame.Zero : AudioLevels.Read(f.Ctx.UtcNow);
            var view = FractalView.Of(o, time, audio, FractalPattern.ShaderIterationCap, surge, quality);
            c.Save();
            ClipBox(c, rect, e.CornerPx);
            c.Translate(rect.Left, rect.Top);
            var drawn = FractalPattern.TryDrawShader(c, f.Sink, o.Kind, cache.FractalColors, in view, box.Width, box.Height,
