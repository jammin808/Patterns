...
public sealed class MediaPattern : IPatternRenderer
{
    public void Render(SKCanvas c, in PatternFrame f)
    {
        ...
        if (o.Source == MediaSource.Playlist)
        {
            ...
            if (now.IsVideo)
            {
                if (Patterns.Core.Services.PlaylistSequencer.IsAudioPath(now.Path))
                {
        ...
        {
            if (Patterns.Core.Services.PlaylistSequencer.IsAudioPath(o.VideoPath))
            {
        ...
        if (o.Source == MediaSource.Web)
        {
            ...
            var name = o.WebUrl.Length > 0 ? Patterns.Core.Services.WebAddress.ShortName(o.WebUrl) : "No web page";
            if (page is null)
            {
...
public sealed class ParticlePattern : IPatternRenderer
{
    public void Render(SKCanvas c, in PatternFrame f)
    {
        ...
        sim.Quality = Patterns.Core.Services.QualityLadder.Shared.Factor;   // the ladder's level: the drawn share, never the field
