...
public sealed class FractalPattern : IPatternRenderer
{
    ...
    public void Render(SKCanvas c, in PatternFrame f)
    {
        ...
        var size = FractalRaster.SizeFor(o.Quality, f.Canvas, Patterns.Core.Services.QualityLadder.RasterScale(quality));
    ...
    private static SKRuntimeEffect? Shader(SinkState sink, FractalKind kind)
    {
        ...
        if (effect is null)
        {
            Patterns.Core.Services.Log.Warn($"Fractal shader ({kind}) unavailable — drawing on the CPU: {errors}");
