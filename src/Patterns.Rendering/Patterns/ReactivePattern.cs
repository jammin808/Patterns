...
public sealed class ReactivePattern : IPatternRenderer
{
    ...
    public void Render(SKCanvas c, in PatternFrame f)
    {
        ...
        var quality = Patterns.Core.Services.QualityLadder.Shared.Factor;
        var drawn = false;
        ...
        if (!drawn)
        {
            var size = ReactiveRaster.SizeFor(o.Quality, f.Canvas, Patterns.Core.Services.QualityLadder.RasterScale(quality));
    ...
    private static SKRuntimeEffect? Shader(SinkState sink, ReactiveScene scene)
    {
        ...
        if (effect is null)
        {
            Patterns.Core.Services.Log.Warn($"Reactive shader ({scene}) unavailable — drawing on the CPU: {errors}");
