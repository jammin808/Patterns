...
public static class HelpTopics
{
    ...
    public static readonly IReadOnlyList<HelpTopic> All = new[]
    {
        ...
        new HelpTopic("show-panel", HelpGroup.RunningTheShow,
            ...
            "CUE GO · CUE STANDBY NEXT / PREV · CUE HOLD ON / OFF · CUE ARM ON / OFF · LOOK <name> · SCREEN <n> LOOK <name> · SCREEN <n> PROGRAM · SCREEN <n> TAKE / CUT · LOCK <n> ON · NEXT / PREV · VIDEO END · VIDEO RESTART · STINGER <name> · LOWERTHIRD <name> · STOPALL",
            new[] { "Panel" },
        ...
        new HelpTopic("callers-pad", HelpGroup.RunningTheShow,
            "The caller's pad, a note on a cue, and the cue row's own menu",
            "SHOW → Run: the pad beside the stack for the day's own words, a note on any cue written in the show, and right-click (or ✎) on a cue row for its verbs.",
            "The Run surface is the caller's, and a caller writes things down: the pad (▸ PAD above the stack) is free words for the day — timings, names, what changed at the rehearsal — saved with the show and read by the assistant before the plan. A note on a cue is the in-show kind: 'Amira wants the lower third held until she sits', 'this one is SKIPPED if we are late' — right-click the row ▸ NOTE… (or press the row's ✎ for a touch screen), type in place, DONE; it shows under the cue on the row and on the standby card in the preview's green, apart from the programmed notes, and travels with the show file. The row's menu is the rest of what a caller reaches for mid-show without leaving the surface: STANDBY HERE (the same as a click), GO THIS CUE NOW (the cue fires whatever is on standby — a jump), SKIP THIS CUE / UNSKIP (GO passes over a skipped cue), and OPEN IN THE CUE EDITOR (the Cues page with that cue selected, for a change the surface cannot make). Why both a right-click and a chip: right-click is the fastest thing a mouse-driven caller has and never competes with the row's one big click (standby), but it is invisible to a first-time user and does not exist on a touch screen — the ✎ keeps the note discoverable and touchable; the heavier verbs stay in the menu so a row never grows a strip of buttons that a fat finger presses by mistake. Typing in the note never moves standby: the box takes the press.",
            new[]
            {
        ...
        new HelpTopic("gods-eye", HelpGroup.RunningTheShow,
            ...
            new[]
            {
                ...
                "After a bad moment press REPLAY: the same picture relit from the journal and the metrics file at an instant — drag the scrub bar or step 30 s to see which screen took what, which cue failed and how the machine was doing then; the rail stays on now; NOW puts the picture of now back.",
            },
            "EYE · EYE FOCUS <screen n | label | id> · EYE NEXT · EYE PREV · EYE LENS <all|video|control|audio|room|problems> · EYE RESET · EYE REPLAY <ON|OFF|time> · EYE AT <time>",
            new[] { "Eye", "Run", "Screens", "Remote" },
            new[] { "god's eye", "gods eye", "eye", "overview", "picture of the show", "topology", "map", "graph", "what is connected", "what is attached", "what feeds", "what drives", "links", "dependencies", "worst", "headline", "problems", "next problem", "red", "amber", "focus", "lens", "zoom", "pan", "bands", "contacts", "hive", "situational awareness", "single pane", "one picture", "companion eye", "eye focus", "replay", "what happened", "scrub", "record", "history of the picture" }),
        new HelpTopic("right-click-menus", HelpGroup.RunningTheShow,
            ...
            new[]
            {
                ...
                "From a deck (round 74): MENU PAGE <name> answers a page's own menu — its looks, cues, designs, people, kinds, presets, media, screens, overlays, tracks and games with their lines, their ticks and the MENU words that open each thing's own menu, and the page's build verbs with * where the deck's text goes (LOOK SAVE *, CUE ADD *, LT NEW * FROM <preset>). NAV <page> [item] turns the desk to the page and selects the thing (a cue, a screen or an element opens its settings column), NAV BACK and NAV HOME walk the desk's page history, NAV SETTINGS ON / OFF / TOGGLE opens or closes the column. The Companion module's Navigator lays all of it on slot keys, so a deck reaches any menu the desk has, and the desk's Remote page and the Eye say where each deck is.",
            },
            "SCREEN <n> PVW LOOK <name> · SCREEN <n> PVW PRESET <name> · SCREEN <n> PVW PATTERN <kind> · SCREEN <n> PVW PROGRAM · SCREEN <n> PVW RESET · PVW LOOK <name> · PVW PATTERN <kind> · PVW RESET · SCREEN <n> PATTERN <kind> · RUN MONITOR <n|PGM|MAIN|OFF> · MENU SCREEN <n> · MENU CUE <number> · MENU LOOK <name> · MENU MONITOR · MENU PAGE <name> · NAV <page> [item] · NAV BACK · NAV HOME · NAV SETTINGS ON|OFF|TOGGLE",
            new[] { "Panel", "Run", "Cues", "Looks", "Lower thirds", "Layers", "Overlays", "Countdown" },
            new[] { "right-click", "right click", "context menu", "menu", "menus", "flyout", "stage", "staged", "pvw", "reset to look", "reset", "preview only", "final say", "drawer", "tile menu", "cue menu", "look menu", "lower third menu", "layer menu", "overlay menu", "wire line", "companion line", "ask the assistant", "page menu", "nav", "navigator", "build verbs" }),
        ...
        new HelpTopic("switcher", HelpGroup.RunningTheShow,
            ...
            "LOOK <name> · SCREEN <n> LOOK <name> · SCREEN <n> PROGRAM · SCREEN <n> TAKE / CUT (the desk's preview to that screen alone, as its own picture) · LOCK <n> ON / OFF · BLACKOUT ON / OFF. TAKE and CUT are desk keys; the phone remote has them too.",
        ...
        new HelpTopic("audio-routing", HelpGroup.RunningTheShow,
            ...
            new[]
            {
                ...
                "Round 69 — the sound follows the picture: Screens page → Sound out names the output each screen's sound leaves by; the matrix derives the route from what the screen shows (the programme, its own picture, the screen it repeats) and moves it on every take. Rows of your own win; AUDIO FOLLOW OFF leaves the rows alone in charge.",
            },
            "AUDIO ROUTING ON|OFF|TOGGLE · AUDIO ROUTE <source> TO <destination> [AT <dB>] · AUDIO UNROUTE <source> FROM <destination> · AUDIO VOG <destination> DUCK|REPLACE|LEAVE · AUDIO FOLLOW ON|OFF|TOGGLE · SCREEN n AUDIO <output>|OFF",
            new[] { "Audio", "Cues", "Screens" },
            new[] { "routing", "route", "matrix", "hdmi", "ndi audio", "embedded audio", "info screen", "soundtrack", "which output", "per output", "sound card", "desk", "vog override", "duck", "replace", "leave", "crosspoint", "db", "level", "trim", "delay", "lip sync", "mixer", "lane", "audio follows video", "afv", "web page audio", "setsinkid", "tone output", "follows the picture", "sound follows", "sound out", "screen audio", "repeater audio", "audio follow" }),
        ...
        new HelpTopic("web-pages", HelpGroup.Content,
            ...
            new[] { "web", "page", "browser", "youtube", "vimeo", "google slides", "office", "powerpoint online", "key", "click", "type", "cursor", "url", "address", "present", "webview", "arm", "armed", "vt", "video", "start point", "mark", "advert", "adverts", "ads", "skip", "play from", "frame rate", "fps", "smooth", "jerky", "screencast", "pre-roll", "clicker", "buffer", "latency", "judder", "stutter", "low latency", "cpu", "webview2", "yt-dlp", "native player", "play via", "libvlc" }),
        ...
        new HelpTopic("edge-blend", HelpGroup.TheRig,
            ...
            new[]
            {
                ...
                "Arrange as a blend grid: columns × rows and the overlap — every screen that is on laid out with the overlaps, on automatic blend; a 2×2 in one press.",
                "Geometry: the corners for the keystone, the edge bends for a curved screen or a lens's bow; the Projection blend pattern's grid to line each projector up with its neighbour.",
                "Black level: a black picture up, then each projector's Black level slider until the brighter bands — and a grid's brighter middle square — go.",
                "Mesh: a lattice of points over the output, each pulled where the picture must land; SHOW ON THE PROJECTOR puts the lattice on the wall while you pull.",
                "Or a camera does it all: Camera calibration — RUN, read the report, APPLY.",
            },
            "",
            new[] { "Screens", "Pattern" },
            new[] { "edge blend", "blend", "projector", "projectors", "overlap", "gamma", "curve", "feather", "soft edge", "grid", "corner", "audit", "2x2", "2×2", "four projectors", "black level", "black levels", "pedestal", "warp", "keystone", "bend", "bends", "curved screen", "cyclorama", "pincushion", "barrel", "mesh", "mesh warp", "lattice", "control points", "dome", "blend grid", "arrange" }),

        new HelpTopic("calibration", HelpGroup.TheRig,
            "Camera calibration: the projectors measured, the meshes and blends solved",
            "SETUP → Screens → Edge blend → Camera calibration: one camera sees the wall, every projector shows structured light, and the solver places, meshes and blends them.",
            HelpBodies.Calibration,
            new[]
            {
                "A camera that sees the whole wall from one spot — a phone with an NDI camera app, or a capture card through NDI Tools — picked under Camera.",
                "RUN: the patterns go out projector by projector while the camera is read; about a minute for two. Nothing moves yet.",
                "Read the report: each projector's fit (a residual under a pixel or two), its coverage, the overlaps, any corner nobody reaches.",
                "APPLY: each projector into its place with its mesh and its blend mask. UNDO puts everything back; the meshes stay editable.",
                "No NDI camera: WRITE PLAN, photograph each pattern as cal-1.png, cal-2.png… with NEXT PATTERN between, then SOLVE FROM PHOTOS.",
                "DEMO reads the report against a room that is not there, without a projector or a camera.",
            },
            "CALIBRATE RUN <camera> · CALIBRATE CANCEL · CALIBRATE DEMO · CALIBRATE APPLY · CALIBRATE UNDO · CALIBRATE STATUS",
            new[] { "Screens" },
        ...
        new HelpTopic("display-unplugged", HelpGroup.TheRig,
            ...
            "Every screen remembers its display — its name, its size, where it sat, its rate — so a display that merely re-indexed (one to its left was unplugged, and Windows renumbered the rest) keeps its screen, and one that is gone is known to be gone. Unplugged: the screen waits, planned and off, with everything programmed for it kept — its position, label, pattern, trims, warp, blend and every look that names it — and the alert goes to the status line, the journal, the health line (SCREEN MISSING: 'Stage left' unplugged at 19:41:58), the super-check's red row and the assistant's brief. The same display back, under whatever id the index gives it, is recognised, adopted and turned on by itself (as it was: a screen that was off stays off). A display the rig has never met, connected while a screen waits, arrives off with a choice on the Screens page: USE AS SUBSTITUTE when its resolution and rate match the missing screen's — or can be forced to (the display offers the size; the mode is switched first, then it stands in) — puts everything programmed for the missing screen on it; ITS OWN SCREEN makes it a new screen, on, and the missing one keeps waiting. A display that cannot stand in (another aspect, a size it cannot reach) is said so in words and can only be its own screen. Adopting by hand on a planned screen still works for anything the watch could not decide.",
            new[]
            {
                "A cable out: the status line and the health line say which screen, since when; the screen waits planned and off with everything kept.",
                "The same display back: adopted and turned on by itself — nothing to press.",
                "A different display: Screens page ▸ A DISPLAY CHANGED — USE AS SUBSTITUTE (forcing the mode first when it must) or ITS OWN SCREEN.",
            },
            "",
            new[] { "Screens" },
            new[] { "unplugged", "unplug", "hot-plug", "hotplug", "hot plug", "cable out", "display lost", "screen missing", "missing display", "plugged back in", "returned", "substitute", "replacement display", "different display", "new display", "stranger", "re-index", "renumbered", "adopt", "planned", "its own screen", "force the mode", "resolution", "frame rate", "aspect" }),
        ...
        new HelpTopic("companion-banks", HelpGroup.Control,
            ...
            "A Stream Deck through Bitfocus Companion 5 (the module finds the desk on the network by itself), or any OSC controller: drag the bank presets once and every look, person, VOG, part, cue and node you make afterwards labels its own key — and the desk drives the deck back.",
            HelpBodies.CompanionBanks,
            new[]
            {
                "Companion 5: import the module package (the companion-module-patterns .tgz the repository's CI builds, or npm run package in integrations/companion-module-patterns), add a Patterns connection and pick this desk under Desk on the network — every Patterns announces itself (Remote page, ANNOUNCE); type the address only on a network that blocks it.",
                "In the connection's settings tick the groups of keys this desk uses — cue stack, transport, looks, patterns, clock, countdown, message, overlays, VOGs, stingers, lower thirds, people, screens, audio, presenter, install, stage, nodes — and the preset list holds only those.",
                "Drag the bank presets — Looks, People, VOGs, Stingers, Parts, Screens, Upcoming cues, Nodes: the keys label themselves and light on air, each kind in its own colour (green on air, amber a preview or a hold, red a lower third or a refusal, a colour per node kind).",
                "The Stage section: the speaker's timer in the timer's own colour with a progress ring, pause and ±1 min, FLASH, WRAP UP and a crew message that stay amber until the stage page ACKs; the Twin section: the role and phase, two-press TAKE OVER and TAKE BACK.",
                "The desk drives the deck: Interactive page → + COMPANION (the address filled in from the Companion heard on the network), the surface id from Companion's Surfaces page in the Surface box, then a cue's Device — send a line with PAGE 3, PRESS 2/0/1 or VAR speaker Jane Doe; the Remote page's COMPANION strip lists every deck connected and its module.",
                "Feedbacks and variables for anything else ($(patterns:desk_version) says which desk a deck is on); OSC gets the same addresses and the same feedback.",
                "The Navigator (round 74, module 3.14.0): one page of keys that reads the desk itself — the rails, then a rail's pages, then a page's own menu (its looks, cues, designs, screens, its build verbs), then a thing's menu and its drawers, each level asked of the desk (NAV, MENU PAGE <name>, MENU <words>) and laid on 24 slot keys in the desk's own colours and ticks; HOME, BACK, PREV / NEXT, RUN / MENU (a thing's key fires its line, or opens its menu), FOLLOW both ways (a page chosen on the deck turns the desk; the desk's page turns the deck) and a TEXT the build verbs take (LOOK SAVE, CUE ADD, PRESET SAVE, LT NEW). Every line gets its own reply, so a refusal lands on the key that asked; the Remote page and the Eye read where each deck is.",
                "The recorder: RECORD ON and the desk's actions arrive at the deck as ACTION lines in the operator's own words (LOOK Walk-in, CUE GO, SCREEN 2 PVW PATTERN Grid), so Companion's action recorder and a key's LEARN write a key from what the operator does on the desk — record the sequence, stop, and the key has it.",
            },
            "NAV <page> [item] · NAV BACK · NAV HOME · NAV SETTINGS ON|OFF|TOGGLE · NAV DECK <words> · MENU PAGE <name> · RECORD ON|OFF · LOOK SAVE <name> · LOOK UPDATE · CUE ADD <name> · PRESET SAVE <name> · LT NEW <name> FROM <preset>; LOOK #<n>, PATTERN <kind>, STAGE, TIMER, TWIN, PLAN and the module's actions and feedbacks; DEVICE Companion PAGE <n> the other way; /patterns/look/index/<n> and /patterns/state/… over OSC.",
            new[] { "Remote", "Interactive" },
            new[] { "companion", "stream deck", "bitfocus", "preset", "bank", "feedback", "variable", "osc", "touchosc", "key", "label", "module", "groups", "tick", "clock", "countdown", "message", "pattern", "mdns", "bonjour", "network", "discover", "stage", "timer", "nodes", "twin", "page", "colour", "color", "palette", "navigator", "nav", "slot keys", "dynamic keys", "record", "action recorder", "learn", "rails", "pages", "menus" }),
        ...
        new HelpTopic("arcade", HelpGroup.Control,
            ...
            new[]
            {
                ...
                "To the wall on this machine: Arcade as a pattern's media, a layer, the PiP's source or a wall tile — straight from the game loop, no NDI between; ARCADE SIZE 3840x1080 shapes it for a joined canvas.",
                "In a window of its own: POP OUT on the page or ARCADE WINDOW on the wire; FULLSCREEN, F11 or ARCADE WINDOW FULL 2 fills a display; Esc brings it back and closes it; the keys are the pads there.",
                "To the wall from another machine: ARCADE NDI ON on the node, then the desk puts the NDI source 'PATTERNS ARCADE (…)' on a screen or a canvas like any other.",
                "Between matches the house plays itself and the board shows; ARCADE NAME ABC signs the last score.",
            },
            "ARCADE START pong 2",
            new[] { "Arcade", "Nodes" },
            new[] { "arcade", "game", "games", "pong", "snake", "breakout", "mini game", "mini games", "attract mode", "leaderboard", "high score", "initials", "pad", "gamepad", "xinput", "controller", "phone pad", "hub", "games hub", "node", "arcade node", "start game", "fixed step", "engine", "arcade window", "pop out", "pop-out", "fullscreen", "full screen", "arcade source", "game on screen", "arcade on the wall" }),
        ...
        new HelpTopic("machine", HelpGroup.TheMachine,
            ...
            new[]
            {
                ...
                "STABILITY: the page switch — every press on the rail timed from the press to the first frame drawn, the worst of the last sixty with where its time went (building the page, the handler, the frame); the super-check's Page switch row goes amber past a desk frame and red past a tenth of a second.",
                "STABILITY: the GO to frame — every GO timed from the press, through the publish its steps made, to the first frame every output drew with it; GO→frame on the glance line, the worst of the last sixty here, the super-check's row amber past 50 ms and red past 100 ms; each sink's word on the glance line carries its lag from a publish to the frame that shows it.",
        ...
        new HelpTopic("twin", HelpGroup.TheMachine,
            ...
            new[]
            {
                "On the main: Role = Main and a port; a main with no key is given one — copy it. On the standby: Role = Standby, the main's address (or leave it empty and let the main's beacon name it), the same key.",
                ...
                "On this machine without lifting a finger: tick 'Run a standby of this desk as a second process' on the main — it starts one in the twin-standby folder beside its own, restarts it when it exits, and ends it on a clean exit unless it has the show.",
                "The hand-back: a main that comes back after its standby took over reads the standby's mark, holds its own outputs (the line says the standby HAS THE SHOW), and TAKE BACK puts the show — with the edits made while the standby ran it — back here: across machines the outputs open here first, the take-back cue points the room here and its boxes answer, and only then does the standby let go; a switch that fails leaves the standby up and TAKE BACK again finishes it.",
                "Refused, with the reason: a takeover on this machine that cannot end the hung main, or cannot mark itself on disk, keeps the outputs held — TAKE OVER ANYWAY (or TWIN TAKEOVER FORCE) overrides by hand, never by itself.",
                "Another machine: 'take over by itself' needs a wall switch cue — a cue that puts that machine's input on the wall through the switcher — because a silence cannot tell a dead main from a cut cable; the take-back cue switches the wall back. Fired is not switched: the cue's boxes must answer (their Confirm level on the Interactive page), and a box that cannot — a UDP datagram — is not a fence for taking over by itself.",
                "A standby on this machine that dies with the show: the main lands what it sent and puts the show back on itself, saying so.",
            },
            "TWIN STATUS · TWIN TAKEOVER · TWIN TAKEOVER FORCE · TWIN STANDBY · TWIN TAKEBACK",
            new[] { "Machine" },
            new[] { "twin", "standby", "hot standby", "redundancy", "redundant", "second process", "dual", "synced", "in step", "mirror", "failover", "take over", "takeover", "take back", "takeback", "hand back", "handback", "local standby", "run a standby", "twin-standby folder", "backup machine", "second machine", "resilience", "endurance", "another folder", "hung main", "main silent", "has the show", "wall switch", "wall-switch cue", "take over anyway", "force", "twin key", "keyless", "split brain", "two desks", "died with the show" }),
