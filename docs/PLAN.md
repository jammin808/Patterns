...
the fields go quiet rather than lying about where something is.

The **Position** picker means what it says. It binds to the place editor, not to the model, so
choosing a position writes the anchor *and clears the nudge*: the element goes to that position on
the screen rather than to that position plus whatever a drag or the sliders had built up. The nudge
then starts again from where the operator put it, which is what a nudge is for. Its sibling is
**RESET TO POSITION** beside the pixel fields — the same clearing without changing the position —
lit only while there is a nudge to drop, and the way out of a drag that went wrong. A drag refreshes
the picker, the pixel fields and RESET as the pointer is let go rather than at the next second's
poll, because that is when the operator is looking at them. So the drag, the picker, the sliders and
the pixels are four ways of saying one thing, and all four move together.
