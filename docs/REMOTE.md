...
| `SCREEN <n> TESTROUTE [ON\|OFF]` (or `TEST ROUTE`) | Round 65.10: the diagnostic profile — 1080p50, RGB, 8-bit, SDR, stereo — stands in for screen *n*'s contract while a path is proven (ON), the contract holds again (OFF), bare toggles; the contract itself is never touched. A soft or wrong picture that clears on the test route is a capability problem, one that stays is a path problem. The screen's signal view carries a Test route line first; the commissioning flow holds at CONTRACT while any screen is on the route. Desk-only: a running order never puts a screen on the test route |
