...
`screens[{n,label,enabled,group,locked,role,armed,own,black}]` (labels honour operator names; `role` is main, confidence, info or repeater; `armed` = the next CUT / TAKE changes it; `own` = it shows a picture of its own, not the program's; `black` = faded to black on its own by `FADE … SCREEN n` and the like), `editSafe` (EDIT SAFE is open: there is a preview and a TAKE to come),
