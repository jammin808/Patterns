...
`web{page,url,title,service,fps,actions[{id,label}],player{pos,dur,text,paused,ad,muted}|null,arm{armed,at,atText,byLook,played,words}|null}` (the web page the program shows — its nickname or host, its address and title, its service when Patterns knows it (YouTube, Vimeo, Google Slides, PowerPoint for the web), the frames it delivered in the last second (a video's rate; 0 for a still page), the actions `WEB KEY <id>` takes on it, what its video player last said (its place and length in seconds and as `1:23 / 4:56`, paused, an advert showing over it) and the arm on it (armed, the mark in seconds and as text, whether the look put it there, whether it has played, the words); `null` with no page on air),
`webArmed{page,key,at,atText,byLook,preRolled,words,short}|null` (the armed web VT anywhere on the desk — a page in the preview, one opened early for a cue ahead (`preRolled`), the page on air — its name, its mark and its words, `short` being `VT armed at 1:23`; `null` with nothing armed),
